...
export type SpawnParticles = {
  /**
   * Trigger 8 converging particles toward a live position reference.
   * `posRef` should be a Vector3 that's mutated each frame (e.g. file.currentPosition)
   * so particles track the moving target.
   */
...
export function createSpawnParticles(scene: Scene): SpawnParticles {
  const geo = new SphereGeometry(0.4, 6, 6); // bigger particles for visibility
  ...
  function trigger(posRef: Vector3, color: number, nowMs: number) {
    ...
    activeGroups.push({ startIdx, targetRef: posRef, startTime: nowMs, dirs, dists });
  }
  ...
  function animate(nowMs: number) {
    ...
    for (let g = activeGroups.length - 1; g >= 0; g--) {
      ...
      if (elapsed > CONVERGE_DURATION_MS) {
        // Done — hide all particles in this group
        for (let i = 0; i < PARTICLES_PER_SPAWN; i++) {
          hideMat.compose(ZERO_POS, IDENTITY_Q, ZERO_SCALE);
      ...
      const t01 = elapsed / CONVERGE_DURATION_MS;
      // Ease-in quadratic → particles accelerate as they converge
      const converge = t01 * t01;
      // Particle scale: starts visible, fades at the end
